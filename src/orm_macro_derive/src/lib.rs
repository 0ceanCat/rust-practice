...
pub fn my_default(input: TokenStream) -> TokenStream {
    ...
    let lines: Vec<String> = fields_map.iter().map(|(k, v)| format!("{} {}", k, v)).collect();
    let lines = lines.join(",\n");

    let keys: Vec<&str> = fields_map.iter().map(|(k, v)| k.as_str()).collect();
    let param_index: Vec<String> = (1..=fields_map.len()).map(|i| format!("?{}", i)).collect();
    let insert_sql = format!("INSERT INTO {} ({}) VALUES ({})", table, keys.join(", "), param_index.join(", "));


    let update: Vec<String> = zip(keys.iter().filter(|x| x.deref().deref() != "id"), &param_index[..param_index.len() - 1])
    ...
    let fields: Vec<Ident> = fields_map.iter().map(|(k, _)| Ident::new(k, Span::call_site())).collect();
    let field_index: Vec<usize> = (0..fields_map.len()).collect();
    let fields_without_id: Vec<Ident> = fields_map.iter().filter(|(k, _)| k.deref() != "id").map(|(k, _)| Ident::new(k, Span::call_site())).collect();

    let select_sql = format!("SELECT {} FROM {}", keys.join(", "), table);
    let gen = quote! {
        impl Entity for #id {
            fn persist(&self) {
                database().execute(#insert_sql, (#(&self.#fields), *));
            }
